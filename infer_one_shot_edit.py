...
import tgs
from tgs.models.renderer_one_shot_edit import GSLayer, make_default_training_args, pcd_from_gaussians
...
class TGS(torch.nn.Module, SaverMixin):
    ...
    def __init__(self, cfg):
        ...
        # compute number of SH channels used by the renderer
        n_sh = (self.renderer.cfg.sh_degree + 1) ** 2
        ...
        # colour modulation weights and biases; size depends on spherical harmonic count
        self.color_w = torch.nn.Parameter(torch.ones(size=(3 * n_sh,)))
        self.color_b = torch.nn.Parameter(torch.zeros(size=(3 * n_sh, 1024, 2048)))
...
class HandLightningModule(pytorch_lightning.LightningModule):
    def __init__(self, cfg: dict, cfg_model: dict, TGS_cfg: dict):
        ...
        for name, param in self.model.named_parameters():
            ...
            if "_frequencies" in name or "_phases" in name or "_amplitudes" in name or "_offsets" in name or "_opacity" in name or "_opacity_volr" in name:
                print(f"param name: {name}, requires_grad: {param.requires_grad}")
                continue
    ...
    def training_step(self, batch, batch_idx):
        ...
        if "comp_rgb_input" in out:
            ...
            # Save a single preview per epoch so you can track visual evolution.
            if batch_idx == 0 and getattr(self, "global_rank", 0) == 0:
                epoch_dir = os.path.join("./vis", expname, "epochs")
                os.makedirs(epoch_dir, exist_ok=True)

                rgb_pred = (
                    out["tex_cal_fine_input"][0]
                    .detach()
                    .clamp(0.0, 1.0)
                    .permute(1, 2, 0)
                    .cpu()
                ...
                cv2.imwrite(
                    os.path.join(epoch_dir, f"epoch_{self.current_epoch:04d}_pred.jpg"),
                    rgb_pred[..., [2, 1, 0]],
                )
                cv2.imwrite(
                    os.path.join(epoch_dir, f"epoch_{self.current_epoch:04d}_ref.jpg"),
                    rgb_input[..., [2, 1, 0]],
                )
    ...
    def preinitialize_neural_primitives(self):
        self.eval()

        # get one batch
        train_loader = self.train_dataloader()
        batch = next(iter(train_loader))
        # manually move batch tensors to same device as model
        ...
        with torch.no_grad():
            self.model._forward(batch)
            self.train()
        return self.model.renderer.gs_net.get_gaussians()
...
if __name__ == "__main__":
    ...
    model = model.cuda()
    g = model.preinitialize_neural_primitives()
